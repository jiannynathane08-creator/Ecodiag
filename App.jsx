    ...
    <div>
      ...
      <div className="result-block">
        ...
        <div className="compare-bars">
          ...
          <div className="compare-bar-row">
            <div className="bar-label"><span>Facturée</span><span>{Math.round(consoFactureeKwh)} kWh</span></div>
            <div className="compare-bar-track">
              <div className="compare-bar-fill facturee" style={{ width: `${(consoFactureeKwh / maxBar) * 100}%` }} />
            </div>
          </div>
        </div>

        {anomalie.anomalie ? (
          <div className="anomaly-flag">
            Écart de {Math.round(anomalie.ecartPct)}% détecté — vérifiez vos équipements
            déclarés ou une possible fuite de consommation.
          </div>
        ) : (
          <div className="ok-flag">Consommation cohérente avec les équipements déclarés.</div>
        )}
      </div>

      <div className="result-block">
        <h3>Installation solaire recommandée</h3>
        <div>
          <span className="big-number amber">{puissanceCreteKwc}</span>
          <span className="unit">kWc</span>
        </div>
        <div className="disclaimer">
          Estimation de coût : {formaterAriary(cout.coutMinAr)} à {formaterAriary(cout.coutMaxAr)}
          {" "}— fourchette large, à faire confirmer par un devis réel d'installateur.
        </div>
      </div>
      ...
      <div className="result-block">
        ...
        <div>
          <span className="big-number good">
            {cout.roiMoisMin ? Math.round(cout.roiMoisMin) : "—"}
            {cout.roiMoisMax ? ` – ${Math.round(cout.roiMoisMax)}` : ""}
          </span>
          <span className="unit">mois</span>
        </div>
        <div className="disclaimer">
          Économie mensuelle estimée : {formaterAriary(cout.economieMensuelleAr)}.
          Ce chiffre suppose une couverture complète de la consommation théorique par le
          solaire — la réalité dépend de l'ensoleillement du jour et de l'usage.
        </div>
      </div>

      <div className="result-actions">
        <button className="btn-primary" onClick={() => window.print()}>Télécharger le rapport (PDF)</button>
        <button className="btn-ghost" onClick={onBack}>Modifier mes réponses</button>
        <button className="btn-ghost" onClick={onRestart}>Nouveau diagnostic</button>
      </div>
    </div>
  );
      }
